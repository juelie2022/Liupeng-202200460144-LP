...
static uint32_t T_PRIME_TABLE[256];
...
static void init_t_tables(void) {
    ...
    for (i = 0; i < 256; i++) {
        ...
        T_PRIME_TABLE[i] = sm4_l_prime_transform(t);
    }
...
/* 使用T表的合成变换T' */
static uint32_t sm4_t_prime_transform_table(uint32_t a) {
    uint8_t a0 = (uint8_t)(a >> 24);
    uint8_t a1 = (uint8_t)(a >> 16);
    uint8_t a2 = (uint8_t)(a >> 8);
    uint8_t a3 = (uint8_t)a;
    
    return T_PRIME_TABLE[a0] ^ 
           rotl32(T_PRIME_TABLE[a1], 8) ^ 
           rotl32(T_PRIME_TABLE[a2], 16) ^ 
           rotl32(T_PRIME_TABLE[a3], 24);
}
...
static void sm4_set_key(SM4_Context *ctx, const uint8_t *key, int is_encrypt) {
    ...
    /* 生成轮密钥 */
    for (i = 0; i < 32; i++) {
        K[i + 4] = K[i] ^ sm4_t_prime_transform_table(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ FIXED_PARAMETER[i]);
        ctx->rk[i] = K[i + 4];
    }
